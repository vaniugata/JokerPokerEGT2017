...
void Game::RenderBonusGame()
{
		...
		m_bonus->GetWinX2().Render(m_renderer, &clip1,
				30, 100, S_BONUSBTN_W, S_BONUSBTN_H);
		SDL_Rect clip2{ 0, 250, S_BONUSBTN_W, S_BONUSBTN_H };
		m_bonus->GetWinX5().Render(m_renderer, &clip2, 30,
				250,S_BONUSBTN_W, S_BONUSBTN_H);
		SDL_Rect clip3{ 0, 400, S_BONUSBTN_W, S_BONUSBTN_H};
		m_bonus->GetWinX10().Render(m_renderer, &clip3, 30,
				400,S_BONUSBTN_W, S_BONUSBTN_H);
		//SDL_Rect clip[2];
...
void Game::ProcessMouseWin() {
		...
		{
			std::cout<<"m_bonus->GetWinX2().IsPressed()"<<std::endl;
		...
		{
			std::cout<<"m_bonus->GetWinX5().IsPressed()"<<std::endl;
			Mix_PlayChannel( -1,m_bonus->ButtonPress, 0 );
			SDL_Delay(1000);
			Mix_PlayChannel( -1,m_bonus->RollDice, 0 );
			int ResultDice = m_bonus->ResultDice();
		...
		{
			std::cout<<"m_bonus->GetWinX10().IsPressed()"<<std::endl;
			Mix_PlayChannel( -1,m_bonus->ButtonPress, 0 );
			SDL_Delay(1000);
			Mix_PlayChannel( -1,m_bonus->RollDice, 0 );
